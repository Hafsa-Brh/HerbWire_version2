...
function EvidenceView({ article }: { article: ApiDiscoveryArticle }) {
  ...
  return <div className="grid gap-7">
    ...
    {article.body_blocks.length < 11 ? <div><p className="hw-eyebrow">Limitations and safety</p><ul className="mt-2 list-disc pl-5 font-sans text-sm text-muted">{article.limitations.map((item) => <li key={item}>{item}</li>)}</ul><p className="mt-3 font-sans text-sm text-muted">{article.safety_context}</p></div> : null}
    {article.geography?.some((item) => item.geography_kind === "botanical_distribution") ? <div><p className="hw-eyebrow">Botanical distribution</p>{article.geography.filter((item) => item.geography_kind === "botanical_distribution").map((item) => <p key={`${item.evidence_type}-${item.display_label}`} className="mt-2 font-sans text-sm text-muted">{item.display_label} / {item.evidence_type.replaceAll("_", " ")} / {item.qualification}</p>)}</div> : null}
    {article.geography?.some((item) => item.geography_kind === "research_geography") ? <div><p className="hw-eyebrow">Research geography</p>{article.geography.filter((item) => item.geography_kind === "research_geography").map((item) => <p key={`${item.evidence_type}-${item.display_label}`} className="mt-2 font-sans text-sm text-muted">{item.display_label} / {item.evidence_type.replaceAll("_", " ")} / {item.qualification}</p>)}</div> : null}
...
export function DiscoveryReviewQueue() {
  ...
  const filteredArticles = articles.filter((article) => {
    ...
    const matchesQuery = `${article.headline} ${identity}`.toLowerCase().includes(query.trim().toLowerCase())
    const matchesBatch = batch === "all" || article.evidence_package.batch_id === batch
    return matchesQuery && matchesBatch
  })
  const pageCount = Math.max(1, Math.ceil(filteredArticles.length / pageSize))
  const safePage = Math.min(page, pageCount)
  const pageArticles = filteredArticles.slice((safePage - 1) * pageSize, safePage * pageSize)
