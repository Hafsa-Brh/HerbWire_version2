...
const navItems: readonly NavItem[] = [
  ...
  { label: "Agent Performance", path: "/admin/performance", icon: Activity },
      ...
      <div className="mx-auto flex max-w-[1320px] items-start px-4 sm:px-6 lg:px-8">
        ...
        <main className="min-w-0 flex-1 py-7 md:ml-[19rem] md:py-10">
          <Routes>
            ...
            <Route path="agents" element={<Navigate to="/admin/performance" replace />} />
            <Route path="runs" element={<PipelineRuns />} />
            <Route path="sources" element={<SourcesCatalogPage />} />
...
function ReviewQueue() {
  ...
  return <><PageHeader eyebrow="Editorial / plant profiles" title="Plants Review" description="Initial plant-profile review remains separate from profile revisions and discovery articles. Human judgment is the final publication gate." action={<button type="button" onClick={data.reload} className="inline-flex items-center gap-2 bg-forest px-4 py-3 font-sans text-xs font-bold uppercase tracking-[.1em] text-cream hover:bg-leaf">Refresh</button>} />{data.data ? <div className="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Review items" value={String(reviews.length)} detail="Profiles in the editorial queue" /><Metric label="Needs attention" value={String(reviews.filter((review) => review.status === "needs_review" || review.status === "held").length)} detail="Drafts or held items" /><Metric label="Approved" value={String(reviews.filter((review) => review.status === "approved").length)} detail="Ready for publication" /><Metric label="Published" value={String(reviews.filter((review) => review.plant_profile?.status === "published").length)} detail="Visible publicly" /></div> : null}{data.isLoading ? <AdminStateCard title="Loading plant reviews" description="Pulling the latest plant profiles waiting for human judgment." /> : null}{data.error ? <AdminStateCard title="Plants Review unavailable" description="The plant-profile review queue could not be loaded right now." action={<button type="button" onClick={data.reload} className="bg-forest px-4 py-3 font-sans text-xs font-bold uppercase tracking-[.1em] text-cream">Try again</button>} /> : null}{data.data ? <ReviewPanel data={data.data} /> : null}</>
}
...
function ReviewPanel({ data }: { data: AdminData }) {
  ...
  return <ReviewWorkspaceShell label="Plants review workspace">
    <Panel eyebrow="Needs attention" title="Plant-profile queue" className="h-full">
...
function ReviewSectionPager({ selectionKey, renderSection }: { selectionKey: string; renderSection: (section: ReviewSectionId) => ReactNode }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <ReviewPagerNav current={sectionIndex + 1} pages={REVIEW_SECTIONS} label="Plant detail pages" onPage={(page) => goTo(page - 1)} headingRef={headingRef} />
    ...
    <ReviewPagerFooter current={sectionIndex + 1} pages={REVIEW_SECTIONS.length} onPage={(page) => goTo(page - 1)} />
  </div>
}
