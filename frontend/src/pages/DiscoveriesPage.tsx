...
export function DiscoveriesPage() {
  ...
  return <SiteShell><main id="top">
    ...
    {discoveries.data && !discoveries.isLoading && !discoveries.error ? <section className="hw-container py-14 md:py-20">
      <div className="flex items-end justify-between border-b-2 border-forest pb-4"><div className="min-w-0"><p className="hw-eyebrow">The research archive</p><h1 className="mt-2 font-serif text-[clamp(2.15rem,8.5vw,3rem)] font-semibold leading-[1.02] tracking-[-.04em] text-deep">TRADITIONAL MEDICINE DISCOVERIES</h1></div><span className="hidden shrink-0 font-serif text-sm italic text-muted sm:block">{discoveries.data.total} published</span></div>
      <form key={searchParams.toString()} onSubmit={submit} role="search" className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        <label className="flex min-h-11 items-center border border-line bg-paper px-4 focus-within:border-leaf sm:col-span-2 lg:col-span-2"><Search size={18} className="mr-3 shrink-0 text-forest" /><span className="sr-only">Search discoveries</span><input name="query" aria-label="Search discoveries" maxLength={120} defaultValue={active.query} className="min-w-0 flex-1 bg-transparent font-sans text-sm text-deep outline-none" placeholder="Headline, plant, journal or PMID" /></label>
        ...
        <button type="submit" className="min-h-11 bg-forest px-5 font-sans text-xs font-bold uppercase tracking-[.1em] text-cream transition hover:bg-leaf lg:col-span-1">Search</button>
        <div className="grid gap-3 sm:col-span-2 sm:grid-cols-2 lg:col-span-2"><FilterSelect name="publication_year" label="Filter by publication year" value={active.publicationYear} options={filters?.publication_years ?? []} placeholder="All years" /><FilterSelect name="research_country" label="Filter by research country" value={active.researchCountry} options={filters?.research_countries ?? []} placeholder="All research countries" /></div>
      ...
      {discoveries.data.items.length ? <nav aria-label="Discovery archive pagination" className="mt-12 grid grid-cols-[auto_1fr_auto] items-center gap-2 border-t border-line pt-5"><button type="button" disabled={discoveries.data.page <= 1} onClick={() => updateParams(active, discoveries.data!.page - 1)} className="inline-flex min-h-11 items-center gap-1 border border-line px-3 font-sans text-[10px] font-bold uppercase tracking-[.08em] text-forest disabled:opacity-35 sm:gap-2 sm:px-4 sm:text-xs sm:tracking-[.1em]"><ChevronLeft size={15} /> Previous</button><span className="text-center font-sans text-[11px] text-muted sm:text-xs">Page {discoveries.data.page} of {discoveries.data.total_pages}</span><button type="button" disabled={discoveries.data.page >= discoveries.data.total_pages} onClick={() => updateParams(active, discoveries.data!.page + 1)} className="inline-flex min-h-11 items-center gap-1 border border-line px-3 font-sans text-[10px] font-bold uppercase tracking-[.08em] text-forest disabled:opacity-35 sm:gap-2 sm:px-4 sm:text-xs sm:tracking-[.1em]">Next <ChevronRight size={15} /></button></nav> : null}
    </section> : null}
