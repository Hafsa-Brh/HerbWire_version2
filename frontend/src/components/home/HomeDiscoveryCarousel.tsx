...
export function HomeDiscoveryCarousel({ items }: { items: ApiPublicDiscoveryArticle[] }) {
  ...
  const normalizedIndex=index%count,article=items[normalizedIndex],image=article.hero_image?.local_path
  ...
  >
    ...
    <div className="flex items-center gap-4 pt-5"><span className="min-w-12 font-sans text-[11px] font-bold text-forest">{String(normalizedIndex+1).padStart(2,"0")} / {String(count).padStart(2,"0")}</span><div className="h-px flex-1 bg-line" aria-hidden="true"><div className="h-px bg-leaf transition-[width] duration-300 motion-reduce:transition-none" style={{width:`${((normalizedIndex+1)/count)*100}%`}}/></div><button type="button" onClick={()=>move(-1)} aria-label="Show previous discovery" className="grid h-11 w-11 place-items-center rounded-full border border-line text-leaf transition hover:border-leaf hover:bg-sage/20"><ArrowLeft size={18}/></button><button type="button" onClick={()=>move(1)} aria-label="Show next discovery" className="grid h-11 w-11 place-items-center rounded-full border border-line text-leaf transition hover:border-leaf hover:bg-sage/20"><ArrowRight size={18}/></button></div>
    <p className="sr-only" aria-live="polite" aria-atomic="true">Slide {normalizedIndex+1} of {count}: {article.headline}</p>
  </section>
}
