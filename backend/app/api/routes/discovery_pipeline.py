"""Authenticated Editorial Desk API for sequential Discovery automation."""

from uuid import UUID

from backend.app.api.discovery_pipeline_schemas import (
    DiscoveryPipelineItemResponse,
    DiscoveryPipelinePreviewResponse,
    DiscoveryPipelineRunResponse,
    DiscoveryPipelineStartRequest,
)
...
router = APIRouter(prefix="/admin/discovery-pipeline")
...
def _response(run: PipelineRun) -> DiscoveryPipelineRunResponse:
    items = [
        DiscoveryPipelineItemResponse(
            id=item.id,
            title=item.discovery_article.headline,
            status=item.status,
            editorial_status=item.discovery_article.status,
            discovery_article_id=item.discovery_article_id,
            review_id=item.review_id,
        )
        for item in sorted(run.discovery_items, key=lambda value: value.position)
        if item.discovery_article is not None
    ]
    return DiscoveryPipelineRunResponse(id=run.id, status=run.status, items=items)


@router.get(
    "/preview",
    response_model=DiscoveryPipelinePreviewResponse,
...
def preview(
    ...
    return DiscoveryPipelinePreviewResponse(
        can_start=can_start,
...
@router.get(
    "/runs/current",
    response_model=DiscoveryPipelineRunResponse | None,
    dependencies=[Depends(require_editor)],
)
...
@router.get(
    "/runs/{run_id}",
    response_model=DiscoveryPipelineRunResponse,
    dependencies=[Depends(require_editor)],
)
def read(run_id: UUID, session: Session = Depends(get_session)):
    run = get_run(session, run_id)
    if run is None:
        raise HTTPException(status_code=404, detail="Discovery pipeline run not found.")
...
def start(
    payload: DiscoveryPipelineStartRequest,
    ...
    try:
        ...
        return _response(run)
    except DiscoveryPipelineBusyError as error:
...
def retry(
    ...
    except DiscoveryPipelineStateError as error:
        raise HTTPException(status_code=409, detail=str(error)) from error
